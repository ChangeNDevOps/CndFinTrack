module Member::PaymentsHelper
  ...
  def breadcrumbs
    ...
    if controller_name == 'payments'
      ...
      result << controller_items[:"member/payments"].merge(path: new_member_company_payment_path(@company), title: 'New paymen') if action_name == 'new'
    end
    result
  end

  def add_button(position)
    controller_items[:'member/payments'].merge(path: new_member_company_payment_path(@company), title: 'Add new paymen') if position == :top and action_name == 'index'
  end
end
